...
public class StickerPackValidator {
    ...
    private static void validateSticker(@NonNull Context context, @NonNull final String identifier, @NonNull final Sticker stickers) throws IllegalStateException {
        if (stickers.emojis.size() > EMOJI_LIMIT) {
            throw new IllegalStateException("emoji count exceed limit, stickers pack identifier:" + identifier + ", filename:" + stickers.imageFileName);
        }
        if (TextUtils.isEmpty(stickers.imageFileName)) {
            throw new IllegalStateException("no file path for stickers, stickers pack identifier:" + identifier);
        }
        validateStickerFile(context, identifier, stickers.imageFileName);
    }
